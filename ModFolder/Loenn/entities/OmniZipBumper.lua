...
OmniZipMover.name = "ChroniaHelper/OmniZipBumper"
...
function OmniZipMover.sprite(room, entity)
    ...
    local bumpTx = "objects/Bumper/Idle00"
    local rectangle = drawableRectangle.fromRectangle("bordered",x,y,width,height,{28,69,212,0.4},{255,255,255,1})
    
    local sprites = {
        --rectangle
        drawableSprite.fromTexture(bumpTx, entity):setPosition(entity.x + entity.width / 2, entity.y + entity.height / 2)
    }
    ...
    for nodeIndex, node in ipairs(nodes) do
        local sprite = drawableSprite.fromTexture(bumpTx, entity):setPosition(node.x + entity.width / 2, node.y + entity.height / 2)
