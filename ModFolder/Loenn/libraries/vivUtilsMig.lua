...
function vivUtilsMig.addAll(receiver, sender, insertLoc)
    if insertLoc then
...
-- Simple functions
function vivUtilsMig.signedModulo(base,modulus) 
    return math.sign(base * modulus) * math.abs(base) % math.abs(modulus)
end

function vivUtilsMig.mod(base,modulus) return (base % modulus + modulus) % modulus end

function vivUtilsMig.copyTexture(baseTexture, x, y, relative)
...
function vivUtilsMig.contains(table, element)
...
function vivUtilsMig.trim(s) return ((string.match(s,'^()%s*$')) and '' or string.match(s,'^%s*(.*%S)')) end

function vivUtilsMig.isNullEmptyOrWhitespace(s)
    if s then
        return #vivUtilsMig.trim(s) == 0
    end return true
end

-- Format for VivHelper is "abgr" (format follows Color.PackedValue)
function vivUtilsMig.getColor(color, allowXNAColors)
    if type(color) == "nil" then 
        return false
    elseif type(color) == "number" then
        return true, (color % 0x100), (vivUtilsMig.rshift(color, 8) % 0x100), (vivUtilsMig.rshift(color,16) % 0x100),(vivUtilsMig.rshift(color,24) % 0x100)
...
function vivUtilsMig.getColorTable(color, allowXNAColors, defaultTable)
    defaultTable = defaultTable or {0,0,0,1}
    local parsed, r, g, b, a = vivUtilsMig.getColor(color, allowXNAColors)
    if parsed then
        return {r,g,b,a or 1}
    else return defaultTable end
end

function vivUtilsMig.invertGetColor(r,g,b,a) -- returns a string in abgr format, such that getColor(invertGetColor(r,g,b,a)) == r,g,b,a is true
...
function vivUtilsMig.swapRGBA(str) -- swaps rgba to abgr and vice versa
    if #str >= 8 then return str:sub(7,8) .. str:sub(5,6) .. str:sub(3,4) .. str:sub(1,2) else return str end
end

function vivUtilsMig.lerp(a,b,t) return a * (1-t) + b * t end

function vivUtilsMig.colorLerp(a,b,lerp)
    local A, ar, ag, ab, aa = vivUtilsMig.getColor(a, true)
    local B, br, bg, bb, ba = vivUtilsMig.getColor(b, true)
    return {vivUtilsMig.lerp(ar, br,lerp), vivUtilsMig.lerp(ag,bg,lerp), vivUtilsMig.lerp(ab,bb,lerp), vivUtilsMig.lerp(aa or 1,ba or 1,lerp)}
end
function vivUtilsMig.alphMult(color, alpha)
    local newColor = {}
...
function vivUtilsMig.indexof(keyvaluetable, toFind)
...
function vivUtilsMig.getNameRotationHandler(rotations)
    return function (room, entity, direction)
        local startIndex = vivUtilsMig.indexof(rotations, entity._name)

        local realIndex = vivUtilsMig.mod(startIndex + direction, 4) + 1
...
function vivUtilsMig.getDataRotationHandler(dataName, rotations)
    return function (room, entity, direction)
        local startIndex = vivUtilsMig.indexof(rotations, entity[dataName])
        local realIndex = vivUtilsMig.mod(startIndex - 1 + direction, 4) + 1
...
function vivUtilsMig.getNameFlipHandler(rotations)
    return function (room, entity, horizontal, vertical)
        local startIndex = vivUtilsMig.indexof(rotations, entity._name)

        if vertical then
            if startIndex == 0 or startIndex == 2 then
                entity._name = rotations[vivUtilsMig.mod((startIndex + 2), 4) + 1]
                return true
            end
        else
...
function vivUtilsMig.getBorder(sprite, color)
    local function get(xOffset,yOffset)
        ...
        texture.color = color and vivUtilsMig.getColorTable(color) or {0, 0, 0, 1}
        return texture
    end
...
function vivUtilsMig.splitToTable(str, pat) 
    local ret = {}
    for d in vivUtilsMig.split(str, pat) do table.insert(ret, d) end
    return ret
end

function vivUtilsMig.containsPredicate(predicate, data)
...
function vivUtilsMig.firstPredicate(predicate, data)
    for _, dataValue in ipairs(data) do
        if predicate(dataValue) then
            return dataValue
        end
...
function vivUtilsMig.GetFilePathWithNoTrailingNumbers(AllowEmpty, atlasName)
    ...
    return {
        ...
        filenameProcessor = function(filename, rawFilename, prefix)
            local str = vivUtilsMig.trim((not filename and "" or filename))
            local a = false
...
function vivUtilsMig.printJustifyText(text, x, y, width, height, font, fontSize, trim, align)
