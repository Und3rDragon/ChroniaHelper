...
namespace ChroniaHelper.Triggers.PolygonSeries {
    public class PolygonCollider : Collider {
        ...
        public override bool Collide(Grid grid) {
            bool ret = false;
            Rectangle PolygonGridBounds = AABB;
            PolygonGridBounds.X -= 7; PolygonGridBounds.Width += 14;
            PolygonGridBounds.Y -= 7; PolygonGridBounds.Height += 14;
            int x0 = (int)Math.Ceiling(PolygonGridBounds.Left / 8.0);
            int x1 = (int) Math.Floor( PolygonGridBounds.Right / 8.0);
            int y0 = (int) Math.Ceiling( PolygonGridBounds.Top / 8.0);
            int y1 = (int) Math.Floor( PolygonGridBounds.Bottom / 8.0);
            for(int x = x0; x < x1; x++) {
                for(int y = y0; y < y1; y++) {
