...
public class SerialImageGroup
{
    ...
    // ---- 每个成员槽位持有独立的 SerialImage（贴图列表取自对应路径），
    //      成员的测量结果互不覆盖，成员自身的对齐基于它自己测量出的矩形 ----
    private readonly List<SerialImage> slotImages = new();

    private SerialImage SlotImage(int i)
    {
        while (slotImages.Count <= i) { slotImages.Add(null); }

        SerialImage image = slotImages[i];
        if (image.IsNull())
        {
            image = new SerialImage(cachedMember[SafeGetPath(i)].textures);
            slotImages[i] = image;
        }
    ...
    private SerialImage PrepareMember(int i)
    {
        SerialImage image = SlotImage(i);
    ...
    private void RebuildLayout()
    {
        ...
        for (int i = 0; i < members.Count; i++)
        {
            ...
            SerialImage previous = members[i - 1];
    ...
    public void Measure<T>(IList<IList<T>> source, Func<T, int> selector)
    {
        ...
        {
            SerialImage image = PrepareMember(i);
