...
public class RefillCancel
{
    public class PlayerIndicator : Entity
    {
        ...
        private static MTexture dashX = GFX.Game["ChroniaHelper/PlayerIndicator/chevron"],
                      dashRefX = GFX.Game["ChroniaHelper/PlayerIndicator/triangle"],
                      stamRefX = GFX.Game["ChroniaHelper/PlayerIndicator/square"];
        ...
        {
            base.Render();
            if (inSpace && Md.Session.ShowIndicator)
            {
                if (DashRestrict) { dashX.DrawCentered(player.BottomCenter + new Vector2(-11f, 6f), Color.Red, 1f); }
                if (DashRefillRestrict) { dashRefX.DrawCentered(player.BottomCenter + new Vector2(0f, 6f), Color.Blue, 1f); }
                if (StaminaRefillRestrict) { stamRefX.DrawCentered(player.BottomCenter + new Vector2(12f, 6f), Color.Yellow, 1f); }
            }
        }
    }
    ...
    private static PlayerDeadBody Player_Die(On.Celeste.Player.orig_Die orig, Player self, Vector2 direction, bool evenIfInvincible, bool registerDeathInStats)
    {
        ...
        if (s != null) //bruh
        {
            Md.Session.staminaCount = self.Stamina = Math.Max(self.Stamina, Md.Session.staminaCount);
    ...
    private static void Player_Update(On.Celeste.Player.orig_Update orig, Player self)
    {
        ...
        if (!self.Scene.Tracker.Entities.ContainsKey(typeof(ShattersongZone))) { orig.Invoke(self); return; }
        lE = self.CollideAll<ShattersongZone>();
        if (inSpace)
        {
            if (p == null || p.Scene == null)
            ...
            {
                if (self.Dashes > Md.Session.dashCount)
                {
                    self.Dashes = Md.Session.dashCount;
                }
                else if (self.Dashes < Md.Session.dashCount)
                {
                    Md.Session.dashCount = self.Dashes;
                }
            }
        }
        ...
        else if (lE.Count > 0 && !inSpace)
        {
            inSpace = true;
            Md.Session.dashCount = self.Dashes;
            Md.Session.staminaCount = self.Stamina;
        }
        orig.Invoke(self);
        if (inSpace)
        {
            DashRestrict = DashRefillRestrict = StaminaRefillRestrict = false;
            foreach (ShattersongZone rcs in lE)
            {
            ...
            if (DashRefillRestrict)
            {
                ...
                if (self.Dashes > Md.Session.dashCount) { self.Dashes = Md.Session.dashCount; } else if (self.Dashes < Md.Session.dashCount) { Md.Session.dashCount = self.Dashes; }
            }
            else
            {
                Md.Session.dashCount = self.Dashes;
            ...
            if (StaminaRefillRestrict && self.Stamina > Md.Session.staminaCount) { self.Stamina = Md.Session.staminaCount; } else { Md.Session.staminaCount = self.Stamina; }
        }
    ...
    {
        List<Entity> lE = self.CollideAll<ShattersongZone>();
        if (lE.Count > 0)
        {
...
[CustomEntity("ChroniaHelper/ShattersongZone")]
[Tracked]
public class ShattersongZone : Entity
{
    ...
    public ShattersongZone(EntityData data, Vector2 offset) : base(data.Position + offset)
    {
        ...
        {
            particles.Add(new Vector2(RandomUtils.Random.NextFloat(width - 1f), RandomUtils.Random.NextFloat(height - 1f)));
        }

        Ldm.LoadHook<RefillCancel>();
    }
        ...
        if (draw)
        {
            ...
            switch (TypeAsInt)
            {
                case 1:
                    ...
                    break;
                case 2:
                    vector.Add(Vector2.UnitY.Rotate(MathF.PI / 3f * 2f));
                    break;
                case 3:
                    vector.Add(Vector2.UnitY.Rotate(MathF.PI / 3f));
                    break;
                case 4:
                    vector.Add(Vector2.UnitY.Rotate(MathF.PI / 3f * 4f));
                    break;
                case 5:
                    vector.Add(Vector2.UnitY.Rotate(MathF.PI / 3f * 5f));
                    break;
                case 6:
                    vector.Add(Vector2.UnitY.Rotate(MathF.PI));
                    break;
                case 7:
                    for (int h = 0; h < 6; h++) { vector.Add(Vector2.UnitY.Rotate(MathF.PI * h / 3f)); }
                    break;
                default:
            ...
            int i = 0;
            for (int count = particles.Count; i < count; i += 1)
            {
    ...
    {
        Draw.Rect(base.Collider, Color.Lerp(color, Color.White, 0.133f) * 0.1f);
        for (int i = 0; i < particles.Count; i += 1)
        {
            ...
            switch (TypeAsInt)
            {
                case 1:
                    ...
                    break;
                case 2:
                    v = Vector2.UnitY.Rotate(MathF.PI / 4f * 3) * 1.414f;
                    break;
                case 3:
                    v = Vector2.UnitY.Rotate(MathF.PI / 4f) * 1.414f;
                    break;
                case 4:
                    v = Vector2.UnitY.Rotate(MathF.PI / 4f * 5) * 1.414f;
                    break;
                case 5:
                    v = Vector2.UnitY.Rotate(MathF.PI / 4f * 7) * 1.414f;
                    break;
                case 6:
                    v = Vector2.UnitY.Rotate(MathF.PI);
                    break;
                default:
                    v = Vector2.Zero;
                    break;
            ...
            if (TypeAsInt == 7)
                Draw.Pixel.Draw(Position + particles[i], Vector2.Zero, new CColor(color).Parsed(0.125f));
            else
                Draw.Line(vec - v, vec + v, new CColor(color).Parsed(0.125f));
        }
    }

    private void BaseDraw()
    {
        Draw.Rect(base.Collider, Color.Lerp(Color.White, color, 0.133f) * 0.1f);
        for (int i = 0; i < particles.Count; i += 1)
        {
            Vector2 vec = Position + particles[i];
            Draw.Pixel.Draw(Position + particles[i], Vector2.Zero, new CColor(color).Parsed(0.125f));
