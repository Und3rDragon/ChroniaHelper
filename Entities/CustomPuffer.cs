...
public class CustomPuffer : Actor
{
    ...
    private CColor outlineColor = CColor.White;
        ...
        : base(position)
    {
        ...
        sprite.Play("idle");
        Add(happySprite = GFX.SpriteBank.Create(data.Attr("happySprite", "pufferFish")));
        ...
        happyFlag = data.Attr("happyFlag", "happy");

        pufferBoopSound = data.Attr("pufferBoopSound", "event:/new_content/game/10_farewell/puffer_boop");
        pufferExplodeSound = data.Attr("pufferExplodeSound", "event:/new_content/game/10_farewell/puffer_splode");
        pufferShrinkSound = data.Attr("pufferShrinkSound", "event:/new_content/game/10_farewell/puffer_shrink");
        pufferReturnSound = data.Attr("pufferReturnSound", "event:/new_content/game/10_farewell/puffer_return");
        pufferExpandSound = data.Attr("PufferExpandSound", "event:/new_content/game/10_farewell/puffer_expand");
        pufferReformSound = data.Attr("pufferReformSound", "event:/new_content/game/10_farewell/puffer_reform");
    }
    /// <summary>
    /// Default = 0 <br/>
    /// True = 1 <br/>
        ...
        : this(data, data.Position + offset, data.Bool("right", false), data.Float("angle", 0f), data.Float("radius", 32f), data.Attr("sprite", "pufferFish"))
    {
        ...
        outlineColor = data.GetChroniaColor("outlineColor", Color.White);
    ...
    {
        base.Added(scene);
        if ((scene as Level).Session.GetFlag(happyFlag))
        {
            isHappy = true;
    ...
    private void GotoIdle()
    {
        if (State == States.Gone)
        {
            ...
            sprite.Play("recover");
            Audio.Play(pufferReformSound, Position);
        }
    ...
    private void GotoHit(Vector2 from, float deg = 0f)
    {
        ...
        Audio.Play(pufferBoopSound, Position);
    }
    ...
    private void Explode()
    {
        ...
        Audio.Play(pufferExplodeSound, Position);
        sprite.Play("explode");
        ...
        foreach (FlagButton entity in Scene.Tracker.GetEntities<FlagButton>())
            if (CollideCheck(entity))
                entity.TurnOn();
    ...
    public override void Render()
    {
        ...
        if ((num > 0f || alwaysShowOutline) && State != States.Gone && State != States.Held)
        {
            ...
            for (int i = 0; i < segments; i++)
            {
                ...
                if (i == 0 || i == segments - 1)
                {
                    Draw.Line(vector, vector - value2 * (blastRadius - 12f), outlineColor.Parsed(t));
                    continue;
                }
                ...
                {
                    Draw.Line(vector, vector - value2 * 3f, outlineColor.Parsed(t));
                }
                else
                {
                    Draw.Point(vector, outlineColor.Parsed(t));
                }
            }
        }
        base.Render();
    ...
    public override void Update()
    {
        ...
        if (!isHappy && SceneAs<Level>().Session.GetFlag(happyFlag))
        {
            isHappy = true;
            sprite.Visible = false;
            happySprite.Visible = true;
        }
        else if (isHappy && !SceneAs<Level>().Session.GetFlag(happyFlag))
        {
            isHappy = false;
            sprite.Visible = true;
            happySprite.Visible = false;
            ...
            case States.Idle:
                {
                    ...
                    {
                        Audio.Play(pufferShrinkSound, Position);
                        sprite.Play("unalert");
                    }
            ...
            case States.Gone:
                {
                    ...
                    if (goneTimer <= 0.5f)
                    {
                        ...
                        {
                            Audio.Play(pufferReturnSound, Position);
                        }
    ...
    private void Alert(bool restart, bool playSfx)
    {
        if (sprite.CurrentAnimationID == "idle")
        {
            if (playSfx)
            {
                Audio.Play(pufferExpandSound, Position);
            }
            sprite.Play("alert");
