...
public class TimerRecordStuff : Entity
{
    ...
    public override void Render()
    {
        ...
        string time = ChroniaHelperModule.Session.TimeRecords.ContainsKey(recordID) ? ConvertTimeToString(TimeSpan.FromTicks(recordTIme), showMilliseconds, showUnits) : "--:--";
    ...
    {
        long time;
        ChroniaHelperModule.Session.TimeRecords.TryGetValue(recordID, out time);
        if (time != default)
            recordTIme = time;
    }
