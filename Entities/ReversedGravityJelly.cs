...
[CustomEntity("ChroniaHelper/ReversedGravityJelly")]
class ReversedGravityJelly : Actor
{
    ...
    public ReversedGravityJelly(EntityData data, Vector2 offset) : base(data.Position + offset)
    {
        ...
        this.diagThrowXMultiplier = data.Float("diagThrowXMultiplier", 1.8f);
        this.gravity = data.Float("gravity", -30);
        startPosition = Position;
        this.canBoostUp = data.Bool("canBoostUp", true);

        downholdSpeed = data.Float("downholdSpeed", -24f);
        windUpholdSpeed = data.Float("windUpholdSpeed", -176f);
        upholdSpeed = data.Float("upholdSpeed", -120f);
        upwindSpeed = data.Float("upwindSpeed", -80f);
        neutralSpeed = data.Float("neutralSpeed", -40f);
        ...
        Add(sprite = GFX.SpriteBank.Create(data.Attr("SpriteXML", "ChroniaHelper_ReversedGravityJelly")));
        Add(wiggler = Wiggler.Create(0.25f, 4, null, false, false));
        Depth = data.Int("Depth", -5);
    ...
    private static IEnumerator OnPickupCoroutine(On.Celeste.Player.orig_PickupCoroutine orig, Player self)
    {
        ...
        if (self.Holding.Entity is not ReversedGravityJelly jelly)
        {
        ...
        if (self.Holding != null && self.Holding.SlowFall)
        {
            ...
            else if (self.gliderBoostTimer > 0f && self.gliderBoostDir.Y < 0 && ((ReversedGravityJelly)self.Holding.Entity).canBoostUp)
            {
                Input.Rumble(RumbleStrength.Medium, RumbleLength.Short);
                self.gliderBoostTimer = 0f;
    ...
    {
        if (self.Holding?.Entity is ReversedGravityJelly && Input.MoveY.Value == 1 && Input.MoveX.Value != 0)
        {
        ...
        else if (self.Holding?.Entity is ReversedGravityJelly && Input.MoveY.Value == -1)
        {
            Input.Rumble(RumbleStrength.Strong, RumbleLength.Short);
            self.Holding.Release(Vector2.UnitX * (float)self.Facing);
            self.Play(SFX.char_mad_crystaltheo_throw, null, 0f);
    ...
    private static void patchPlayerNormalUpdate(ILContext il)
    {
        ...
        if (cursor.TryGotoNext(MoveType.After, instr => instr.MatchLdcR4(120f)))
        {
            ...
            cursor.EmitDelegate<Func<float, Player, float>>((speed, player) => {
                ...
                if (player?.Holding?.Entity is ReversedGravityJelly jelly)
                {
                    if (player.SceneAs<Level>().Wind.Y > 0)
                        return jelly.downholdSpeed + 40;
                    return jelly.downholdSpeed;
                }
                return speed;
            });
        }
