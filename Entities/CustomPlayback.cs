...
namespace ChroniaHelper.Entities
{
    ...
    // Source code from VivHelper Github repository

    [CustomEntity("ChroniaHelper/CustomPlayback")]
    [Tracked]
    class CustomPlayback : Entity, IPreAwake
    {
        ...
        public CustomPlayback(EntityData e, Vector2 offset)
            : this(e.Position + offset,
                  e.Enum<PlayerSpriteMode>("sprite", PlayerSpriteMode.Playback),
                  e.Attr("tutorial"))
        {
            ...
            color = null;
            if (e.Attr("Color") != "")
                color = Calc.HexToColor(e.Attr("Color"));
            ...
            alwaysActive = string.IsNullOrEmpty(e.Attr("activeFlag"));
            alwaysDisplay = string.IsNullOrEmpty(e.Attr("displayFlag"));
            activeFlag = e.Attr("activeFlag");
            displayFlag = e.Attr("displayFlag");
        }
        private bool alwaysActive, alwaysDisplay;
        private string displayFlag, activeFlag;

        public CustomPlayback(Vector2 start, PlayerSpriteMode sprite, string tutorial)
        {
        ...
        public override void Update()
        {
            ...
            // whether it should listen to the display flag
            if (alwaysActive) { active = true; }
            else
            {
                active = MapProcessor.level.Session.GetFlag(activeFlag);
            }
            ...
            if (active)
            {
                if (index >= Timeline.Count - 1 || Time >= TrimEnd)
                {
                    ...
                    bool display;
                    if (alwaysDisplay) { display = true; }
                    else
                    {
                        display = MapProcessor.level.Session.GetFlag(displayFlag);
                    }
                    if (loopDelay <= 0f && display)
                    {
                        ...
                        if (!alwaysDisplay)
                        {
                            MapProcessor.level.Session.SetFlag(displayFlag, false);
                        }
                    }
                }
