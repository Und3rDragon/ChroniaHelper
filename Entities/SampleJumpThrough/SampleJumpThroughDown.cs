...
public class SampleJumpThroughDown : JumpThru {
    ...
    public static void activateHooks() {
        ...
        using (new DetourConfigContext(new DetourConfig("ChroniaHelper", before: new[] { "*" })).Use()) {
            ...
            canUnDuckHook = new Hook(typeof(Player).GetMethod("get_CanUnDuck"), typeof(SampleJumpThroughDown).GetMethod("modCanUnDuck", BindingFlags.NonPublic | BindingFlags.Static));
        }
    }
            ...
            if (cursor2.TryGotoNext(MoveType.AfterLabel, instr => instr.MatchLdarg(0), instr => instr.MatchLdflda<Actor>("movementCounter"))) {
                ...
                cursor.EmitDelegate<Func<Actor, int, JumpThru>>((self, moveV) => {
                    ...
                    if (moveV < 0 && !self.IgnoreJumpThrus) {
                        JumpThru jumpthru = self.CollideFirstOutside<SampleJumpThroughDown>(self.Position + Vector2.UnitY * moveDirection);
                        if (jumpthru != null) {
        ...
        if (cursor.TryGotoNext(MoveType.AfterLabel, instr => instr.MatchLdarg(1), instr => instr.MatchLdcI4(0))) {
            ...
            if (cursor2.TryGotoNext(MoveType.After, instr => instr.MatchLdloc(localPlatform.Index), instr => instr.OpCode == OpCodes.Brtrue_S)) {
                ...
                cursor.EmitDelegate<Func<Platform, int, Platform>>((self, moveV) => {
                    int moveDirection = Math.Sign(moveV);
                    if (moveV < 0) {
                        return self.CollideFirstOutside<SampleJumpThroughDown>(self.Position + Vector2.UnitY * moveDirection);
                    }
                    return null;
                });
    ...
    private static int updateClimbMove(Player player, int lastClimbMove) {
        if (Input.MoveY.Value == -1 && player.CollideCheckOutside<SampleJumpThroughDown>(player.Position - Vector2.UnitY)) {
            player.Speed.Y = 0;
            return 0;
        }
    ...
    private static void onPlayerOnCollideV(On.Celeste.Player.orig_OnCollideV orig, Player self, CollisionData data) {
        ...
        if (self.StateMachine.State != 19 && self.StateMachine.State != 3 && self.StateMachine.State != 9 && playerMovingUp(self)
            && self.CollideCheckOutside<SampleJumpThroughDown>(self.Position - Vector2.UnitY)) {
                ...
                switch ((nextInstruction.Operand as MethodReference)?.FullName ?? "") {
                    case "T Monocle.Entity::CollideFirstOutside<Celeste.JumpThru>(Microsoft.Xna.Framework.Vector2)":
                        ...
                        cursor.EmitDelegate<Func<JumpThru, JumpThru>>(jumpThru => {
                            if (jumpThru?.GetType() == typeof(SampleJumpThroughDown))
                                return null;
                            return jumpThru;
                        });
                        break;
                    case "System.Boolean Monocle.Entity::CollideCheckOutside<Celeste.JumpThru>(Microsoft.Xna.Framework.Vector2)":
                        ...
                        cursor.EmitDelegate<Func<bool, Entity, Vector2, bool>>((orig, self, at) => orig && !self.CollideCheckOutside<SampleJumpThroughDown>(at));
                        break;
                    case "System.Boolean Monocle.Entity::CollideCheck<Celeste.JumpThru>()":
                        ...
                        cursor.EmitDelegate<Func<bool, Player, bool>>((vanillaCheck, self) => vanillaCheck && !self.CollideCheck<SampleJumpThroughDown>());
                        break;
                    ...
                    case "System.Collections.Generic.List`1<Monocle.Entity> Monocle.Tracker::GetEntities<Celeste.JumpThru>()":
                        ...
                        cursor.EmitDelegate<Func<List<Entity>, List<Entity>>>(matches => {
                            for (int i = 0; i < matches.Count; i++) {
                                if (matches[i].GetType() == typeof(SampleJumpThroughDown)) {
                                    matches.RemoveAt(i);
                                    i--;
                                }
                            }
    ...
    private static void onPlayerUpdate(On.Celeste.Player.orig_Update orig, Player self) {
        bool unduckWouldGoThroughPlatform = self.Ducking && !self.CollideCheck<SampleJumpThroughDown>();
        if (unduckWouldGoThroughPlatform) {
            Collider bak = self.Collider;
            self.Collider = normalHitbox;
            unduckWouldGoThroughPlatform = self.CollideCheck<SampleJumpThroughDown>();
            self.Collider = bak;
        }
        ...
        if (unduckWouldGoThroughPlatform && !self.Ducking) {
            ...
            while (self.CollideCheck<SampleJumpThroughDown>() && !self.CollideCheck<Solid>(self.Position + new Vector2(0f, 1f))) {
                self.Position.Y++;
            }
        }
    }
    ...
    private static bool modCanUnDuck(orig_get_CanUnDuck orig, Player self) {
        ...
        if (!result || !self.Ducking || self.CollideCheck<SampleJumpThroughDown>()) {
            return result;
        }
        ...
        self.Collider = normalHitbox;
        if (self.CollideCheck<SampleJumpThroughDown>()) {
            ...
            result = !self.CollideCheck<SampleJumpThroughDown>();
        }
    ...
    public SampleJumpThroughDown(EntityData data, Vector2 offset)
        : base(data.Position + offset, data.Width, true) {

        Ldm.LoadHook(typeof(SampleJumpThroughDown));
    ...
    public override void Awake(Scene scene) {
        ...
        Point[] blocks = new Point[columns];
        ...
        if (animationDelay > 0f) {
            if (atlasFrames.Length > 0) {
                Add(new JumpThroughAnimator(
                    atlasFrames, columns, horizontal: true, animationDelay, blocks,
                    flipY: true, yOffset: 8f)
                {
                    Depth = Depth
                });
            }
    ...
    public override void MoveVExact(int move) {
        bool playerInverted = APIGravityHelper.playerInverted;
    ...
    private static bool onJumpthruHasPlayerRider(On.Celeste.JumpThru.orig_HasPlayerRider orig, JumpThru self) {
        if (self is SampleJumpThroughDown) {
