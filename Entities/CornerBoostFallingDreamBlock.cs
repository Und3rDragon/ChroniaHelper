...
public class CornerBoostFallingDreamBlock : CustomDreamBlock
{
        ...
        : base(data, offset)
    {
        ...
        Add(new SolidModifierComponent(2, false, false));
    }
    ...
    private IEnumerator Sequence()
    {
        ...
        while (true)
        {
            ...
            for (int i = 2; i < Width; i += 4)
            {
                ...
                {
                    SceneAs<Level>().Particles.Emit(Celeste.FallingBlock.P_FallDustA, 2, new Vector2(X + i, Y), Vector2.One * 4f, (float)Math.PI / 2f);
                }
                SceneAs<Level>().Particles.Emit(Celeste.FallingBlock.P_FallDustB, 2, new Vector2(X + i, Y), Vector2.One * 4f);
            }
            FallSFX();
    ...
    private void LandParticles()
    {
        for (int i = 2; i <= Width; i += 4)
        {
            ...
            {
                SceneAs<Level>().ParticlesFG.Emit(Celeste.FallingBlock.P_FallDustA, 1, new Vector2(X + i, Bottom), Vector2.One * 4f, -(float)Math.PI / 2f);
                float direction = (!(i < Width / 2f)) ? 0f : ((float)Math.PI);
                SceneAs<Level>().ParticlesFG.Emit(Celeste.FallingBlock.P_LandDust, 1, new Vector2(X + i, Bottom), Vector2.One * 4f, direction);
            }
        }
    }
