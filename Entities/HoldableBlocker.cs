...
public class HoldableBlocker : Solid
{
    ...
    private readonly List<HoldableBlocker> adjacent = new List<HoldableBlocker>();
    ...
    private HoldableBlocker(Vector2 position, float width, float height, EntityData data)
        : base(position, width, height, false)
    {
        ...
        Selected = data.StringArray("holdableTypes");
    }
    private string[] Selected;

    public HoldableBlocker(EntityData data, Vector2 offset)
    ...
    private void OnReflect()
    {
        ...
        foreach (HoldableBlocker seekerBarrier in adjacent)
        {
    ...
    {
        On.Celeste.Holdable.Update += TheoCrystalOnUpdate;
    ...
    {
        if (data.Hit is HoldableBlocker barrier)
        {
            barrier.OnReflect();
        }
    }

    private static void TheoCrystalOnUpdate(On.Celeste.Holdable.orig_Update orig, Holdable self)
    {
        List<Entity> theoCrystalBarrier = self.Scene.Tracker.GetEntities<HoldableBlocker>().ToList();
    ...
    {
        List<Entity> theoCrystalBarrier = self.Scene.Tracker.GetEntities<HoldableBlocker>().ToList();
        foreach (var barrier in theoCrystalBarrier)
        {
            if ((barrier as HoldableBlocker).Selected.Contains(self.Holding?.Entity.GetType().Name))
            {
    ...
    {
        List<Entity> barriers = self.Scene.Tracker.GetEntities<HoldableBlocker>();
    ...
    {
        HoldableBlocker barrier = player.CollideFirstOutside<HoldableBlocker>(player.Position + direction);
        if (barrier == null)
        {
            barrier = player.CollideFirst<HoldableBlocker>();
    ...
    private static bool PlayerOnPickup(On.Celeste.Player.orig_Pickup orig, Player self, Holdable pickup)
    {
        ...
        var theoCrystalBarrier = self.Scene.Tracker.GetEntities<HoldableBlocker>().ToList();
        foreach (var barrier in theoCrystalBarrier)
        {
            barrier.Collidable = true;

            var collide = self.CollideCheck(barrier);

            if (collide && (barrier as HoldableBlocker).Selected.Contains(pickup.Entity.GetType().Name))
            {
                result = false;
            }
            else
            {
                result = orig(self, pickup);
            }
