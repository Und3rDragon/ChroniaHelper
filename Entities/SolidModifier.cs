...
[Credits("Original code from VivHelper")]
[CustomEntity("ChroniaHelper/SolidModifier")]
public class SolidModifier : Entity
{
    ...
    public SolidModifier(EntityData data, Vector2 offset) : base(data.Position + offset)
    {
        ...
        else
        {
            ...
            foreach (string s in strings)
            {
                if (s.StartsWith("*"))
                {
                    Type t = Utils.TypeUtils.GetType(s.Substring(1), false);
                    if (t != null)
                    {
                ...
                {
                    Type t = Utils.TypeUtils.GetType(s, false);
                    if (t != null)
                    {
                        Types.Add(t);
                    }
    ...
    public override void Awake(Scene scene)
    {
        ...
        foreach (Solid solid in CollideAll<Solid>())
        {
            ...
            if (Utils.TypeUtils.MatchTypeFromTypeSet(t, Types, assignableTypes)) //Added the count check because of a weird bug.
            {
                if (solid is SolidTiles st &&
                    Utils.Miscs.GridRectIntersection(st.Grid, new Rectangle((int)Top, (int)Left, (int)Width, (int)Height), out Grid overlap, out Rectangle scope))
                {
